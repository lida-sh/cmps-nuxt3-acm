...
export const useAddToCartService = () => {
    ...
    return (course_id:number) => {
        return fetchData("/cart/add", { params: { expand: "course.src,course.statusText,course.isRecording,course.percent" }, method: "post", body: {course_id} }, {setToken:true, beforeResponse, toastValidationFields: ["course_id"]})}
}
export const useDeleteCartService = ()=>{
    const fetchData = useFetchApi<boolean>()
    return (course_id)=>{
        return fetchData("/cart/delete",{params: {course_id}, method: "delete"}, {setToken:true, ignorErrors:true})
    }
}